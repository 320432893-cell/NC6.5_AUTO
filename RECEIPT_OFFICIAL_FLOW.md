...
```bash
/mnt/h/python脚本/.venv/nc_auto_v2/.venv-local/Scripts/python.exe tools/receipt_full_flow_entry.py --post-query-only --query-after-save
```

该模式只根据 Sheet2 当前待补行重建查询目标，打开或使用 NC 查询页，查询已保存收款单并写回 Sheet2。它不执行录入、不保存、不点新增收款单，用于 NC 查询结果首次为空、人工关闭重进后重新补跑查询。
...
补做后验查询复用第 4、5 步的查询与写回逻辑，但目标行来自 Sheet2，不重新读取 Sheet1 生成完整录入计划；GUI 必须把当前列映射和文本字段映射传给引擎，保证 Sheet2 行和页面配置能对上。
...
```text
开单点击自制 -> 从当前 canvas/缓存取得表头 scope -> 用 `财务组织(O)` 锚点确认当前录入页 -> 语义定位首批表头字段并学习本次模板 -> 后续字段优先复用已学习路径 -> 失败时先做弹窗故障判断并重试当前动作，仍失败再做受限语义定位/诊断
```

旧“稳定后缀 path 直接写字段”和 `finance-org-write-path` 探针已经废弃。财务组织的价值是确认当前 canvas/scope，并尽量为后续字段缩小语义搜索范围；它不再提供一个可长期缓存的写入 path。表头字段定位以当前页面实测语义结果为准，成功后学习本次模板用于同批复用。

稳定事实：

- 当前页 path 前缀会随打开的 NC 窗口/页签数量变化。
- 同类收款单页面内，字段相对结构通常稳定，但不能把旧固定后缀当主路。
- 不能把 `.2/.3` 这类前缀段当长期真相。
...
- 当前代码以 `财务组织(O)` 锚点建立当前 scope/cache；客户、商务领款备忘、备注等字段优先使用学习到的表头模板，未命中时走受限语义定位。不要重新恢复旧固定 path 瞎撞。
- 主路 path 失败时，先判断是否被可取消 Java 弹窗打断；如是，聚焦弹窗并 `Alt+C` 后重试当前失败动作。
- 不是弹窗导致的 path 失败时，只允许当前字段受限语义定位/诊断接管；不得回退鼠标、bounds 中心点点击或表头账户参照旧方案。
...
- 默认每个字段按明细表 path 重新定位目标单元格；只有配置了 `focus_via_col` 的特殊字段才先定位入口列，再用方向键进入目标列。
- 字段值用剪贴板整段粘贴，然后 `Enter` 确认。
- 收款银行账户列必须从相邻 `col=5` 方向键进入 `col=4`，粘贴裸账号后等待约 `0.1s` 再 `Enter`，让 NC 匹配候选并确认落格。
- 主行和手续费行的正式字段都启用即时 verify；读回不匹配时按同款写入流程补写一次，再交给后台 verifier 做最终闭包。
...
- 主行写完后保留或新增第 2 行给手续费。
- 第 2 行只有为空或已确认是手续费行时才允许覆盖；如果未成功覆盖为手续费业务类型，当前单据会被取消并跳过，后续行继续执行，Sheet2 留给人工核对。
...
Sheet2 写回前会先清理当前行旧样式，再根据本次结果设置成功/失败展示；因此补做后验查询成功的行会恢复正常底色，不保留旧失败红色。
...
- 连续保存成功。
- 保存后按主体统一查询。
- Sheet2 写入业务列、NC客户名称、后验核对状态和异常原因。
- Sheet2 按主体排序，每个主体数据块末尾追加主体合计行，汇总原始金额、手续费和到账金额。
