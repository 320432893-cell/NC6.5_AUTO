...
- Windows/H 盘运行目录：
  `H:\python脚本\.venv\nc_auto_v2`
- JAB 必须使用 Windows Python 跑，不能直接用 WSL/Linux Python 跑：
  `H:\python脚本\.venv\nc_auto_v2\.venv-local\Scripts\python.exe`
- 当前 Excel 配置：
...
- `tools/jab_batch.py plan`
  - 读取 Excel `Sheet1`。
  - 跳过第二列已有状态/凭证号的行。
  - 解析 A 列 `金额+对手方` 拼接 key。
  - 读取当前 NC 待生成主表。
  - 按 `金额 + 对手方` 唯一匹配 NC 行。
  - 只做只读规划，不点击 NC。

- `tools/jab_batch.py generate --yes`
  - 正确逻辑：在待生成主表一次性选中 Excel 剩余可匹配行。
  - 只点击一次 `生成 -> 前台生成`。
  - 在同一个 `制单` 窗口里按 Excel 行顺序查找制单表记录并保存。
  - 每次保存后验证制单表行数减少、目标行消失，或制单表为空。
  - 制单表为空时关闭制单窗口，回待生成表 F5 刷新，确认本轮记录都消失。
  - 保存后先写入 `已生成待回填`，后续由 backfill 写正式凭证号。

- `tools/jab_batch.py switch-generated`
  - 从待生成界面按 F3 打开查询。
  - 选择 `正式单据`。
  - 点击 `确定`。
  - 切换到已生成列表并读取表格确认。

- `tools/jab_batch.py backfill`
  - 在已生成列表中按 `金额 + 对手方` 匹配 Excel 行。
  - 已生成表存在历史重复时，优先取凭证日期等于当天的记录。
  - 读取凭证号列，去掉前导 0。
  - 校验凭证号范围：`1 <= 凭证号 <= 9999`。
  - 写回 Excel 第二列。

- `tools/jab_batch.py split-keys`
  - 把 A 列 `金额+对手方` 拼接 key 拆成独立两列。
  - 默认 C 列写金额，D 列写对手方。
  - 不改 A 列原始 key，不改 B 列凭证号。

- `core/jab_operator.py`
  - 封装 JAB 启动、控件点击、表格读取、表格多选、窗口关闭、F3/F5 按键。
  - 已加入隐藏空白蓝框/小 AWT 浮窗逻辑。
...
- 前 3 行成功回填历史生成凭证号。
- 第 5-8 行小批量测试成功。
- 第 9-10 行验证“制单窗口还在但制单表为空”的正常完成状态。
- 第 11-34 行按正确逻辑测试：
  - 待生成主表一次性选中 24 条。
  - 只点一次 `生成 -> 前台生成`。
  - 制单窗口出现 24 条。
...
- 不要把待生成主表按递增拆批后反复点 `生成 -> 前台生成`。
  - 这是已经踩过的错。
  - 如果只选 1 行去前台生成，NC 的制单界面可能和多行场景不同，JAB 读表也会变。
  - 正确做法是：待生成主表全量选中，一次前台生成。

- 制单窗口内可以按递增制单行号批量保存，但凭证号顺序可能不等于 Excel 顺序。
  - 已发生案例：
    `excel_rows=[25, 26] voucher_rows=[1, 9]`
  - 回填结果：
    `Excel行25 -> 370`
    `Excel行26 -> 369`
  - 结论：如果凭证号必须严格按 Excel 顺序递增，制单窗口内不能多行批量保存，应改成一行一保存。

- 不能只信“保存成功”提示。
  - 最终证据应该是制单表目标行消失、制单表行数减少，或制单表为空后关闭并刷新待生成表确认记录消失。

- 制单窗口还在但表空白是正常完成状态。
  - 程序应关闭制单窗口。
  - 然后回待生成表 F5 刷新复核。

- Excel 文件被 WPS/Excel 打开时，写回会失败。
  - 错误是：
    `PermissionError: [Errno 13] Permission denied`
  - `backfill`、`split-keys`、生成状态写入都会受影响。

- PowerShell 多行命令不能直接换行写参数。
  - 要么写一行。
  - 要么用 PowerShell 反引号续行。

- JAB path/hwnd 不稳定，不要长期硬编码 path。
  - 正式逻辑应按 role/name/窗口标题动态搜索。

- `backfill` 当前假设已经在已生成/正式单据列表。
  - 如果不确定，先跑 `switch-generated`。

### 当前优先 TODO

1. 如果用户要求凭证号严格按 Excel 顺序递增：
   - 修改制单保存策略为一行一保存。
   - 禁用当前制单窗口内的递增多行批量保存。
...
2. 如果用户优先要求速度：
   - 保留待生成全量选择。
   - 实验制单窗口大批量/全量保存。
   - 事后按已生成表真实凭证号回填，不要求凭证号递增。

3. 重构前先补 `models.py` 和 `contracts.py`：
   - 把现在的 dict 数据结构改成 dataclass。
   - 把散落的 RuntimeError 改成有上下文的契约错误。
