...
- NC/JAB 表格列位统一按 0 基索引；Excel/openpyxl 列号是 1 基。收款单结果表已确认名称列 `2`、金额列 `7`；`8` 是旧的 1 基误填值，配置校验会拦截。
...
1. 收款单查询闭环
   ...
   - 当前真实 NC 已跑通 `659 = 500 + 159` 分页采集，且 `nc_rows=659` 已进入 dry-run；后续重点是确认名称列和金额列口径。

2. Excel 回写
   - 按 Excel 候选行的 `原始金额 + 银行来款名` 匹配 NC 的 `原币金额 + 名称列`；日期只用于查询/候选范围，不参与匹配。
   - 匹配唯一时写 `是否NC已做过`；未找到写未做过；重复命中不执行并报告“名称和金额相同，需人工确认”。
...
- 不要手写/复用裸坐标；若 JAB 动作接口找不到控件，只允许按同一 JAB path 做 bounds 点击回退，并在报告里记录点击方法。
