...
默认录入路径必须优先走后台 JAB：`setTextContents`、JAB action、selection/table API 和 JAB 控件树读回。前台窗口只用于判断能不能发送全局键盘输入，不能用来否定已经由后台 JAB 写入并回显成功的业务状态。明细表是当前例外：正式代码先用 JAB 按表格 path 选中目标字段或入口字段并校验前台窗口，再用受保护剪贴板整段粘贴、`Enter` 确认、后台表格读回校验；只有配置了 `focus_via_col` 的字段才从相邻入口列用方向键进入。
...
收款单页签路径里的 `.2/.3` 这类段位会随当前打开的 NC 界面数量变化；用户打开另一个界面后当前收款页可能从 `.2` 变成 `.3`。这不是业务状态，也不是失败原因，不能把固定页签索引当长期依据。正式逻辑用 `财务组织(O)` 锚点建立当前 canvas/scope 缓存，再按当前页面语义定位并学习本次表头字段模板；后续字段优先复用已学习结果，失败才做受限语义定位，不再依赖旧固定后缀瞎撞。

表头字段正式定位口径是“当前 canvas + `财务组织(O)` 锚点确认 scope + 字段语义定位学习 + 本批复用”。`finance-org-write-path` 这类财务组织写入 path 探针已删除，不作为正式逻辑或缓存来源。字段定位失败时先判断是否被可取消 Java 弹窗打断，命中则聚焦弹窗并 `Alt+C` 后重试当前动作；仍失败时走当前字段的受限语义定位/诊断，不回退表头账户参照、鼠标坐标或 bounds 中心点点击。
...
明细表当前已验证的填写顺序：主行写 `收款业务类型=货款`、`收款银行账户`、`科目=1002`、`贷方原币金额`、`结算方式=网银`。币种只写表头，输入 `USD`/`CNY` 后回车；主行和手续费行不写币种。正式写入方式是：每个字段按明细表 path 重新定位目标单元格；账号列和手续费账户清空列因 NC 联动特殊，先定位相邻入口列 `col=5` 再用方向键进入 `col=4`。字段值整段剪贴板粘贴后 `Enter` 确认；账号列粘贴裸账号后等待 `0.1s` 再 `Enter`，让 NC 完成候选匹配。主行和手续费行所有正式字段都启用即时读回 verify，失败时按同款写入流程补写一次，并由后台 pipeline verifier 做最终闭包。表头 `结算方式` 必须显式写 `网银`，不能依赖主体默认值；明细主行和手续费行的 `结算方式` 输入后用 Enter 确认，避免联想浮层残留。手续费非零时才 `Ctrl+I` 增行，手续费行写 `手续费`、`660305`、手续费金额、`网银`；手续费行账户必须为空，自动带出时用 `Delete` 清空；若多出空白第 3 行，用 `Ctrl+D` 删除。

表头字段提交和校验口径：财务组织编码、客户编码、单据日期、币种、表头结算方式按当前 scope/学习到的字段定位写入后提交，现场验证口径是输入后 Enter 再进入下一字段。即时 JAB `setTextContents` 返回成功或同控件读回为空都不是业务 oracle；当前保存前闭包依赖表头写入返回、明细后台 verifier、保存结果和后验查询，不做逐字段同步总扫描。收款银行账户由明细账号带出并由后台 verifier 校验；表头账户 JAB 读回为空只记 warning，不作为保存前硬失败。
...
- 当前正式明细策略：每个字段按明细表 path 重新定位目标单元格；只有字段配置了 `focus_via_col` 时，才先定位入口列再用 Left/Right 方向键进入目标列。所有非清空字段使用整段剪贴板粘贴，普通字段粘贴后短等待再 Enter，账号列粘贴后等待 `0.1s` 再 Enter 让 NC 完成候选匹配。主行和手续费行字段均有即时 verify，失败后补写一次；后台 verifier 继续做批量闭包。`setTextContents`、逐字符 typing、直接双击/聚焦粘贴账号列都不是当前主路径。
