...
`tools/receipt_self_made_flow.py` 默认只负责开单和表头阶段；表头字段主路是“当前 canvas + 动态前缀 + 固定后缀 path”写入，`财务组织(O)` 是前缀硬锚点。完整流程开单成功后立即写表头首字段；下方表格 path 预热只在财务组织写入成功后后台启动，不得阻塞财务组织写入。`receipt_self_made_flow` 是探针入口，不承担明细填入（开单+表头完成后只读一次明细表供检视；旧 `--fill-detail` 后台写法已按取代纪律删除）；明细正式入口统一调用 `tools/receipt_detail_*`，使用受保护前台键盘和剪贴板粘贴，不能回退到坐标或无守卫 typing。
