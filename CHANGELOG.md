...
## 2026-07-01 - 凭证回填重跑、收款补做后验查询和监控清理

- 凭证生成后的回填改为可单独重跑：`backfill` / GUI“匹配凭证号”只重新查询 NC 已生成列表并回填 Excel，不生成、不保存凭证。C 列只跳过真实数值凭证号，`已生成待回填`、错误文字、空值等状态仍可参与回填，避免刷新慢或首次查询空表后无法补救。
- 已生成结果表定位删除旧的工具栏/页面状态脆弱判断，改为 `core/nc_generated_result_locator.py`：先按单据生成模块动态前缀定位 scope，再在 scope 内确认含真实凭证号列的结果表。凭证号仍以第 22 列真实有效号为唯一硬条件，不用列数、日期或工具栏文字证明页面。
- `generate-and-backfill` 在写 Excel 状态前增加 Excel 写入预检，文件被 WPS/Excel 占用时提前报 `ExcelLockedError`，GUI 可直接提示“关闭 Excel 后再执行”，不再等 NC 已保存后才暴露权限错误。
- 收款单新增 `--post-query-only` / GUI“补做后验查询”：基于 Sheet2 当前待补/异常行重建查询目标，只打开或使用 NC 查询页，查询已保存收款单并写回 Sheet2；不录入、不保存、不点新增。该入口用于 NC 查询结果首次为空、人工关闭重进后补跑。
- Sheet2 写回统一先清除旧行样式再写当前结果；后验匹配成功的行会恢复正常样式，不再保留上次失败的红色底色。表头/明细核验失败原因继续细化到字段，例如 `备注 expected=... actual=...`。
- 收款单客户名称匹配放宽长短名称包含关系，如 `TINGCO SAS` / `TING`、`UK FUELS LIMITED T/AS ...` / `UK FUELS LIMITED`；但短名称在长名称中重复出现的高风险模式继续不放行，保留 `AZUGA INC ... AZUGA INC` / `AZUGA INC` 失败口径，避免误匹配。
- 新增收款单/凭证运行监控脚本和 bat 入口，重点观察 JAB 存活、Java/Python 进程数、内存、线程存活、异步 verifier 提示和 run_state 阶段卡顿；监控只读，不参与业务动作。
- 清理旧凭证 path 稳定性临时探针，正式逻辑不再依赖人工探测脚本输出；JAB `ensure_started` 防御补强，避免未初始化 DLL 时按 path 查找直接抛 `NoneType.isJavaWindow`。

## 2026-06-29 - 收款单录入稳妥校验、后验查询和死逻辑清理

- 明细写入正式走“定位 -> 写入 -> 读回”稳妥链路，主行关键字段继续即时校验；`收款银行账户`、`科目` 这类 NC 反应慢的字段允许最多 3 次重写与稍长等待，避免偶发贴不上直接误判。已删除早期不确定的 dynamic path 试探和只读 readback probe 脚本，正式流程不再做全局瞎蒙。
- 汇率改为正式明细校验口径：NC 自动带出的汇率不手写覆盖，但必须读回并归一化比较；CNY/RMB/人民币必须等于 1，USD 等外币必须在合理区间，读不到或不合规则失败并走既有取消/收尾循环。
- 表头统一校验收敛：财务组织视为阻塞式前置字段，不再在统一校验里反复读；币种按 `USD/美元`、`CNY/人民币` 等价归一判断。往来对象优先通过 API 选择为“客户”，API 后仍不是客户才硬失败并取消，避免供应商/部门/业务员等错误对象继续保存。
- 明细读回提速：读取已定位表格时支持上下文缓存与按列/按单元格读取，避免每个字段重复扫描整棵控件树；保留字段级即时校验，优先稳妥，不改成无校验批量盲填。
- 保存后统一后验查询继续作为落库 oracle：按主体和日期范围查询，查询结果先设置每页 `500`，再触发 F5 刷新并等待结果稳定，然后读表匹配。金额匹配使用 Decimal 归一到 2 位小数；金额匹配但名称不匹配、名称匹配但金额不匹配时，异常原因会带用于匹配的名称、期望金额和 NC 同金额/同名称候选值，方便人工判断。
- 后验匹配口径明确：金额和名称唯一匹配才算成功；只有金额匹配或只有名称匹配仍失败；重复同名同金额按重复条数失败。按单据号排序可作为人工核验辅助，但自动匹配不以“最新/最大单据号”绕过重复风险。
