...
def build_interactive_args(
    ...
    *,
    ...
    print()
    start_row_prompt = "从 Sheet1 第几行开始"
    if default_start_row:
        start_row_prompt += f"，默认 {default_start_row}: "
    else:
        start_row_prompt += ": "
    start_row = ask(start_row_prompt, default=default_start_row)
    limit = ask(f"做几条，默认 {default_limit}: ", default=default_limit)
    start_delay = ask(
        f"启动前等待秒数，默认 {default_start_delay}: ",
        default=default_start_delay,
    )

    return ["--start-row", start_row, "--limit", limit, "--start-delay", start_delay]
