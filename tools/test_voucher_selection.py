...
def main():
    ...
    try:
        target = None
        for index, (context, vm_id, owned, info, window) in enumerate(jab.find_tables_once()):
            if window.get("title") == "制单" and info.rowCount > 1 and info.columnCount == 13:
        ...
        selected = jab.get_selected_child_indexes(vm_id, context, info.rowCount * info.columnCount)
