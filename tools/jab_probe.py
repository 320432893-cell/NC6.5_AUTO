...
def iter_candidate_dlls():
    dll_name = "WindowsAccessBridge-64.dll" if platform.architecture()[0] == "64bit" else "WindowsAccessBridge-32.dll"
...
def configure_jab(dll):
    if hasattr(dll, "initializeAccessBridge"):
        ...
        raise RuntimeError("Access Bridge DLL has no initializeAccessBridge or Windows_run export.")
    ...
    if hasattr(dll, "isAccessibleChildSelectedFromContext"):
        dll.isAccessibleChildSelectedFromContext.argtypes = [ctypes.c_long, JOBJECT, ctypes.c_int]
        dll.isAccessibleChildSelectedFromContext.restype = wintypes.BOOL

    if hasattr(dll, "addAccessibleSelectionFromContext"):
        dll.addAccessibleSelectionFromContext.argtypes = [ctypes.c_long, JOBJECT, ctypes.c_int]
        dll.addAccessibleSelectionFromContext.restype = None

    if hasattr(dll, "removeAccessibleSelectionFromContext"):
        dll.removeAccessibleSelectionFromContext.argtypes = [ctypes.c_long, JOBJECT, ctypes.c_int]
...
def enum_windows(include_children=False):
    ...
    def add_window(hwnd):
        ...
        if title or class_name:
            windows.append((hwnd, title, class_name, pid.value, bool(user32.IsWindowVisible(hwnd))))
...
def print_context_tree(dll, vm_id, context, depth=0, max_depth=4, max_children=40, query=None, path="0"):
    ...
    for index in range(count):
        ...
        if child:
            print_context_tree(dll, vm_id, child, depth + 1, max_depth, max_children, query=query, path=f"{path}.{index}")
            if hasattr(dll, "releaseJavaObject"):
...
def print_table_info(dll, vm_id, context, max_rows=5, max_cols=10):
    ...
    for row in range(row_limit):
        for col in range(col_limit):
            ...
            if not dll.getAccessibleTableCellInfo(vm_id, context, row, col, ctypes.byref(cell_info)):
            ...
            if cell_context:
                info = AccessibleContextInfo()
                if dll.getAccessibleContextInfo(vm_id, cell_context, ctypes.byref(info)):
                    name = info.name.strip()
                    desc = info.description.strip()
                    role = info.role_en_US.strip() or info.role.strip()
                    bounds = f"{info.x},{info.y},{info.width},{info.height}"
                    ...
                    print(f"  cell[{row},{col}] context={cell_context} <context info failed>")
            else:
...
def print_actions(dll, vm_id, context):
    ...
    names = [actions.actionInfo[index].name.strip() for index in range(actions.actionsCount)]
...
def do_action(dll, vm_id, context, action_name=None):
    ...
    ok = dll.doAccessibleActions(vm_id, context, ctypes.byref(todo), ctypes.byref(failure))
...
def do_selection_change(dll, vm_id, context, args):
    if args.clear_selection_path:
        if not hasattr(dll, "clearAccessibleSelectionFromContext"):
            print("This Access Bridge DLL does not expose clearAccessibleSelectionFromContext.")
            return False
    ...
    if args.select_all_path:
        if not hasattr(dll, "selectAllAccessibleSelectionFromContext"):
            print("This Access Bridge DLL does not expose selectAllAccessibleSelectionFromContext.")
            return False
    ...
    if args.select_child_index is not None:
        if not hasattr(dll, "addAccessibleSelectionFromContext"):
            print("This Access Bridge DLL does not expose addAccessibleSelectionFromContext.")
            return False
    ...
    if args.remove_child_index is not None:
        if not hasattr(dll, "removeAccessibleSelectionFromContext"):
            print("This Access Bridge DLL does not expose removeAccessibleSelectionFromContext.")
            return False
        dll.removeAccessibleSelectionFromContext(vm_id, context, args.remove_child_index)
...
def main():
    parser = argparse.ArgumentParser(description="Probe Java Access Bridge windows and controls.")
    parser.add_argument("--title", default="NC", help="Only inspect windows whose title contains this text.")
    parser.add_argument("--all", action="store_true", help="Inspect all visible Java windows.")
    parser.add_argument("--children", action="store_true", help="Also inspect child and hidden windows.")
    parser.add_argument("--hwnd", type=int, action="append", help="Inspect a specific window handle.")
    parser.add_argument("--dll", help="Full path to WindowsAccessBridge-64.dll or WindowsAccessBridge-32.dll.")
    parser.add_argument("--depth", type=int, default=4, help="Control tree depth.")
    parser.add_argument("--max-children", type=int, default=40, help="Max children per node.")
    parser.add_argument("--startup-wait", type=float, default=3.0, help="Seconds to wait after initializing Access Bridge.")
    parser.add_argument("--query", help="Only print control lines containing this text.")
    parser.add_argument("--inspect-path", help="Inspect one context path from each matching Java window.")
    parser.add_argument("--actions-path", help="List actions for one context path from each matching Java window.")
    parser.add_argument("--do-action-path", help="Execute an action for one context path from each matching Java window.")
    parser.add_argument("--action", help="Action name to execute. Defaults to the first exposed action.")
    parser.add_argument("--selection-path", help="Inspect selection state for one context path from each matching Java window.")
    parser.add_argument("--select-child-index", type=int, help="Add one child index to the selection for --selection-path.")
    parser.add_argument("--remove-child-index", type=int, help="Remove one child index from the selection for --selection-path.")
    parser.add_argument("--clear-selection-path", help="Clear selection for one context path from each matching Java window.")
    parser.add_argument("--select-all-path", help="Select all children for one context path from each matching Java window.")
    ...
    if hasattr(dll, "initializeAccessBridge"):
        if not dll.initializeAccessBridge():
            print("initializeAccessBridge returned false. Try `jabswitch -enable`, then restart NC and rerun.")
            return 2
    else:
        pump_thread = threading.Thread(target=run_windows_access_bridge, args=(dll, stop_pump), daemon=True)
        pump_thread.start()
    ...
    if args.hwnd:
        known = {int(hwnd) for hwnd, *_ in windows}
        for value in args.hwnd:
            if value not in known:
                ...
                pid = wintypes.DWORD()
                user32.GetWindowThreadProcessId(hwnd, ctypes.byref(pid))
                windows.append((hwnd, title_buffer.value, class_buffer.value, pid.value, bool(user32.IsWindowVisible(hwnd))))
    ...
    for hwnd, title, class_name, pid, visible in windows:
        ...
        if not is_java:
            continue
        if not args.all and not title_match:
            continue

        matched += 1
        print("=" * 80)
        print(f"hwnd={int(hwnd)} pid={pid} visible={visible} class={class_name!r} title={title!r}")

        vm_id = ctypes.c_long()
        context = JOBJECT()
        if not dll.getAccessibleContextFromHWND(hwnd, ctypes.byref(vm_id), ctypes.byref(context)):
            print("Could not get accessible context from this window.")
            continue

        action_path = args.actions_path or args.do_action_path
        selection_path = args.selection_path or args.clear_selection_path or args.select_all_path
        if args.inspect_path or action_path or selection_path:
            target_path = args.inspect_path or action_path or selection_path
            target_context, owned_contexts = get_context_by_path(dll, vm_id.value, context.value, target_path)
            if target_context:
                print(f"Inspecting path {target_path}:")
                print_context_tree(dll, vm_id.value, target_context, max_depth=1, max_children=args.max_children)
                ...
                if selection_path:
                    print_selection_info(dll, vm_id.value, target_context, max_children=args.max_children)
                    if (
    ...
    if matched == 0:
        ...
        if java_candidates:
            ...
            for hwnd, title, class_name, pid, visible in java_candidates[:50]:
                print(f"  hwnd={int(hwnd)} pid={pid} visible={visible} class={class_name!r} title={title!r}")
        print("Checks: NC is open, Java Access Bridge is enabled, and this script uses the same bitness as Java.")
