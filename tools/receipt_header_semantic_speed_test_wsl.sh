...
# 允许依赖层：cmd.exe、Windows py launcher、tools/receipt_self_made_fill_trial.py
...
cmd.exe /c "set PYTHONIOENCODING=utf-8&& cd /d ${WIN_ROOT}&& py -3.11 tools\receipt_self_made_fill_trial.py --probe-header-semantic-field \"${LABEL}\" --probe-timeout ${TIMEOUT} --probe-repeat ${REPEAT} --json"
